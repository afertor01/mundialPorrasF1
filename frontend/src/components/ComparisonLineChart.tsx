...
const ComparisonLineChart: React.FC<Props> = ({ fullData, currentUser, gps, isLogarithmic = false }) => {
    ...
    const playedGps = useMemo(() => {
        ...
        // El backend ya filtra para devolver solo datos de GPs con resultados
        // Aquí filtramos por GPs que tengan datos y los ordenamos por fecha
        const gpsWithData = gps.filter(gp => {
            return Object.values(fullData).some(userData =>
                userData.some(point => point.gp_id === gp.id)
            );
        });
        return gpsWithData.sort((a, b) => new Date(a.race_datetime).getTime() - new Date(b.race_datetime).getTime());
    }, [gps, fullData]);
    ...
    useEffect(() => {
        // Solo establecer usuarios iniciales si no hay ninguno seleccionado
    ...
    }, [currentUser, rankedUsers, allUsers, selectedUsers.length]); // Añadido selectedUsers.length

    // Resetear usuarios seleccionados cuando cambia el modo (logarítmico/linear)
    useEffect(() => {
        setSelectedUsers([]);
    }, [isLogarithmic]);
        ...
        return playedGps.map((gp) => {
            ...
            selectedUsers.forEach((username) => {
                ...
                if (userPoints) {
                    ...
                    if (p) {
                        const val = p.value;
                        // En escala logarítmica, valores <= 0 rompen el gráfico
                        // Para multiplicadores, el mínimo debería ser 1 (neutro)
                        if (isLogarithmic) {
                            point[username] = val <= 0 ? 1 : val;
                        } else {
                            point[username] = val;
                        }
                    } else {
                        // Si no hay dato, dejamos undefined (no null) para que Recharts lo ignore
                        point[username] = undefined;
                    }
                }
            });
            return point;
        });
    }, [fullData, playedGps, selectedUsers, isLogarithmic]);

    // DEBUG: Ver qué datos tenemos
    useEffect(() => {
        console.log('ComparisonLineChart DEBUG:', {
            isLogarithmic,
            playedGpsCount: playedGps.length,
            selectedUsersCount: selectedUsers.length,
            chartDataLength: chartData.length,
            sampleChartData: chartData[0],
            fullDataKeys: Object.keys(fullData).length
        });
    }, [chartData, isLogarithmic, playedGps, selectedUsers, fullData]);
    ...
    const topPadding = isLogarithmic ? 0 : maxVal * 0.1; // Log scale handles padding differently usually
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <YAxis
                            ...
                            domain={isLogarithmic ? [1, 'auto'] : [0, maxVal + topPadding]}
