...
const Admin: React.FC = () => {
  ...
  // ------------------------------------------
  // TAB: ZONA DE PÁNICO (RECALCULAR TODO)
  // ------------------------------------------
  const PanicTab = () => {
    ...
    const handlePanicRebuild = async () => {
      ...
      try {
        ...
        setFeedback({ type: "success", msg: "✅ Reconstrucción completada. Todos los logros y estadísticas han sido recalculados correctamente." });
        setConfirmText("");
      } catch (e: any) {
        setFeedback({ type: "error", msg: "❌ Error crítico durante la reconstrucción: " + (e.response?.data?.detail || e.message) });
      } finally {
