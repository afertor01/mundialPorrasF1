...
const UserSearch = ({ currentUser, onSelectUser, usersList }: any) => {
    ...
    return (
        <div ref={wrapperRef} className="relative w-full md:w-64 z-[100]">
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -5 }}
                        className="absolute top-full mt-2 w-full bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden max-h-72 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent z-[110]"
                    >
                        <button
...
const SettingsTab = ({ avatars, avatar, handleSelectAvatar, loadingAvatar, reloadUser }: any) => {
    const { username, acronym, login } = useContext(AuthContext) as any;
    ...
    const handleSave = async () => {
        try {
            ...
            if (formData.acronym && formData.acronym !== acronym) payload.acronym = formData.acronym;
            if (formData.new_password) {
                if (!formData.current_password) { toast("Debes introducir tu contraseña actual.", "warning"); return; }
                if (formData.new_password === formData.current_password) { toast("La nueva contraseña no puede ser igual.", "warning"); return; }
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Acrónimo</label><input name="acronym" maxLength={3} placeholder={acronym} onChange={handleChange} className="w-full mt-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-700 outline-none uppercase" /></div>
                </div>
    ...
    const metricDefinitions = [
        {
            key: "Regularidad",
            label: "Regularidad",
            desc: "Calculada mediante la varianza de tus puntos. Un nivel alto indica que tus resultados son muy estables y predecibles (poca varianza), mientras que un nivel bajo refleja una trayectoria irregular con grandes altibajos entre carreras.",
            color: "text-blue-500"
        },
        {
            key: "Compromiso",
            label: "Compromiso",
            desc: "Es el ratio entre GPs proyectados y carreras disputadas desde que te uniste. Es alto si no te pierdes ninguna cita del calendario y bajo si sueles olvidar enviar tus predicciones con frecuencia.",
            color: "text-purple-500"
        },
        {
            key: "Anticipación",
            label: "Anticipación",
            desc: "Se basa en el tiempo medio que transcurre desde que guardas tu porra hasta que empieza la carrera. Los pilotos con valor alto planifican con días de antelación, mientras que los valores bajos indican predicciones de 'último minuto'.",
            color: "text-green-500"
        },
        {
            key: "Calidad/Podios",
            label: "Calidad",
            desc: "Media de puntos ponderada por la competitividad de cada GP (nº de participantes). Un valor alto significa que consigues muchos puntos incluso cuando hay muchos rivales, demostrando una alta eficiencia competitiva.",
            color: "text-yellow-500"
        },
        {
            key: "Vidente",
            label: "Vidente",
            desc: "Representa tu porcentaje de aciertos exactos (posición perfecta del piloto o evento clavado). Es alto si tienes una precisión milimétrica en tus pronósticos y bajo si sueles fallar por poco en el orden de llegada.",
            color: "text-red-500"
        }
    ];
...
export default function Profile() {
    ...
    const handleSelectAvatar = async (filename: string) => {
        ...
        try { await API.updateMyAvatar(filename); if (refreshProfile) await refreshProfile(); } catch (e) { toast("Error al cambiar de avatar.", "error"); }
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto pb-20 p-4">
            <div className="relative text-white p-6 md:p-8 rounded-[2.5rem] shadow-2xl border border-gray-800 bg-gray-900 z-30">
