...
const Predictions: React.FC = () => {
    ...
    const handleOpenGp = async (gp: any) => {
        ...
        if (isClosed && !hasPred) {
            toast("El plazo para este Gran Premio ha finalizado.", "warning");
            return;
        }
        setLoadingGp(true);
        setSelectedGp(gp);
    ...
    const handleSave = async () => {
        ...
        if (filledPositions.length < 10) return toast("Completa las 10 posiciones.", "warning");
        if (filledPositions.length !== uniqueDrivers.size) return toast("Has repetido pilotos.", "warning");
        if (!events.FASTEST_LAP) return toast("Selecciona la Vuelta Rápida.", "warning");

        try {
            await API.savePrediction(selectedGp.id, positions, events);
            toast("Predicción guardada correctamente.", "success");
            setExistingPreds(prev => ({ ...prev, [selectedGp.id]: true }));
            setSelectedGp(null);
        } catch (err: any) { toast("Error al guardar la predicción.", "error"); }
    };
                        ...
                        {gps.map((gp, idx) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`relative bg-white rounded-3xl p-6 shadow-sm border-2 transition-all cursor-pointer overflow-hidden ${!canView ? 'opacity-60 grayscale bg-gray-100 border-gray-200' :
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start mb-6">
                                        <span className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${!isOpen ? 'bg-gray-200 text-gray-600' : hasPred ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                                            }`}>
                                    ...
                                    {canView && (
                                        <div className={`text-center py-3 rounded-2xl font-black text-xs uppercase tracking-tighter transition-colors ${isOpen ? (hasPred ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-f1-red text-white shadow-lg shadow-red-200') : 'bg-gray-800 text-white'
                                            }`}>
                                        ...
                                        {[...Array(10)].map((_, i) => {
                                            ...
                                            return (
                                                <div key={pos} className="group flex items-center gap-4 bg-gray-50 p-2 rounded-2xl hover:bg-gray-100 transition-colors">
                                                    <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shadow-sm ${pos === 1 ? 'bg-yellow-400 text-white' :
                                            ...
                                            <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                                {qualyResults.map((driverName: string, idx: number) => (
                                                    <div key={idx} className="flex items-center bg-white p-3 rounded-xl shadow-sm border border-indigo-50">
                                                        <span className={`w-6 h-6 flex items-center justify-center rounded-md text-[10px] font-black mr-3 ${idx === 0 ? 'bg-yellow-400 text-white' :
