...
class BingoRepository:
    ...
    def _calculate_tile_value(
        self, total_participants: int, selections_count: int
    ) -> int:
        """
    ...
    def toggle_selection(self, tile_id: int, current_user: Users) -> Dict[str, str]:
        ...
        if existing:
            ...
            query = select(BingoSelections.bingo_tile_id).where(
                BingoSelections.user_id == current_user.id
            )
            ...
            if current_count >= MAX_SELECTIONS:
                raise HTTPException(
                    status_code=400,
                    detail=f"Has alcanzado el límite de {MAX_SELECTIONS} selecciones.",
                )
