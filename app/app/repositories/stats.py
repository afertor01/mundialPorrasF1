...
class StatsRepository:
    ...
    def evolution(
        ...
        if type == "users":
            ...
            for user in items:
                query = select(Predictions).join(GrandPrix).filter(
    ...
    def ranking(
        ...
        query = select(GrandPrix).filter(GrandPrix.season_id == season_id).order_by(GrandPrix.race_datetime)
        ...
        query = select(RaceResults.gp_id).filter(RaceResults.gp_id.in_([gp.id for gp in gps]))
        ...
        if type == "users":
            ...
            ranking_by_gp = {}
            for gp_id in gp_ids:
                preds = self.session.exec(select(Predictions).filter(Predictions.gp_id==gp_id)).all()
                gp_ranking = []
            ...
            teams = self.session.exec(select(Teams).filter(Teams.season_id==season_id)).all()
            if not teams:
                return {"by_gp": {}, "overall": []}
            ...
            for gp_id in gp_ids:
                ...
                for t in teams:
                    ...
                    preds = self.session.exec(select(Predictions).filter(
                        Predictions.user_id.in_(member_ids),
    ...
    def _calculate_stats(self, target_user_id: int) -> Dict[str, Any]:
        ...
        part_counts = self.session.exec(select(Predictions.gp_id, func.count(Predictions.user_id)).group_by(Predictions.gp_id)).all()
        ...
        metrics_raw = []
        for u in all_users:
            u_preds = self.session.exec(select(Predictions).filter(Predictions.user_id == u.id)).all()
        ...
        target_preds = self.session.exec(select(Predictions).filter(Predictions.user_id == target_user_id)).all()
        # Ordenar
        ...
        for p in target_preds:
            better = self.session.exec(select(func.count(Predictions.id)).filter(Predictions.gp_id == p.gp_id, Predictions.points > p.points)).scalar()
        ...
        if races_played > 0:
            # Hero
            hero = (self.session.exec(select(PredictionPositions.driver_name, func.count(PredictionPositions.driver_name).label('c'))
                    .join(Predictions).filter(Predictions.user_id == target_user_id, PredictionPositions.position <= 3)
                    .group_by(PredictionPositions.driver_name).order_by(desc('c'))).first())
            if hero: insights["hero"] = {"code": hero[0], "count": hero[1]}
            
            # Villain
            villain = (self.session.exec(select(PredictionEvents.value, func.count(PredictionEvents.value).label('c'))
                    .join(Predictions).filter(Predictions.user_id == target_user_id, PredictionEvents.event_type == "DNF_DRIVER")
                    .group_by(PredictionEvents.value).order_by(desc('c'))).first())
            ...
            if bgp:
                ...
                worse = self.session.exec(select(func.count(Predictions.id)).filter(Predictions.gp_id == best_p.gp_id, Predictions.points < best_p.points)).scalar()
                perc = 100
                if total_in_race > 1:
        ...
        if metrics_raw:
            ...
            if my_m:
                radar_data = [
                    {"subject": "Regularidad", "A": self._normalize_score(my_m["reg"], min(regs), max(regs), reverse=True), "fullMark": 100},
                    {"subject": "Compromiso", "A": self._normalize_score(my_m["com"], min(coms), max(coms)), "fullMark": 100},
                    {"subject": "Anticipación", "A": self._normalize_score(my_m["ant"], min(ants), max(ants)), "fullMark": 100},
                    {"subject": "Calidad/Podios", "A": self._normalize_score(my_m["pod"], min(pods), max(pods)), "fullMark": 100},
                    {"subject": "Vidente", "A": self._normalize_score(my_m["vid"], min(vids), max(vids)), "fullMark": 100}
                ]
    ...
    def get_all_users_light(self):
        ...
        query = select(Users.id, Users.username, Users.acronym, Users.avatar, Users.created_at)
