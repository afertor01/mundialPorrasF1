...
class StatsRepository:
    ...
    def evolution(
        ...
        if type == "users":
            ...
            if ids and names:
                query = query.filter(or_(Users.id.in_(ids), Users.username.in_(names)))
            elif ids:
                query = query.filter(Users.id.in_(ids))
            elif names:
                query = query.filter(Users.username.in_(names))
            ...
            for user in items:
                query = (
                    ...
                    .join(GrandPrix)
                    .filter(
                        Predictions.user_id == user.id, GrandPrix.season_id == season_id
                    )
                    .order_by(GrandPrix.race_datetime)
                )
                preds = self.session.exec(query).all()
                ...
                for p in preds:
                    ...
                    evolution_list.append(
                        {"gp_id": p.gp_id, "value": round(acc_value, 4)}
                    )

                response[user.username] = evolution_list

        elif type == "teams":
            query = select(Teams).filter(Teams.season_id == season_id)

            if ids and names:
                query = query.filter(or_(Teams.id.in_(ids), Teams.name.in_(names)))
            elif ids:
                query = query.filter(Teams.id.in_(ids))
            elif names:
                query = query.filter(Teams.name.in_(names))
            ...
            for team in items:
                ...
                query = (
                    ...
                    .join(GrandPrix)
                    .filter(
                        Predictions.user_id.in_(member_ids),
                        GrandPrix.season_id == season_id,
                    )
                    .order_by(GrandPrix.race_datetime, Predictions.user_id)
                )
                preds = self.session.exec(query).all()
                ...
                for gp_id in sorted(gp_map.keys()):
                    ...
                    evolution_list.append(
                        {"gp_id": gp_id, "value": round(acc_value, 4)}
                    )
    ...
    def ranking(
        ...
        query = select(RaceResults.gp_id).filter(
            RaceResults.gp_id.in_([gp.id for gp in gps])
        )
        completed_gp_ids = {res.gp_id for res in self.session.exec(query).all()}
        ...
        if type == "users":
            ...
            ranking_by_gp = {}
            for gp_id in gp_ids:
                preds = self.session.exec(
                    select(Predictions).filter(Predictions.gp_id == gp_id)
                ).all()
                gp_ranking = []
            ...
            teams = self.session.exec(
                select(Teams).filter(Teams.season_id == season_id)
            ).all()
            if not teams:
                return {"by_gp": {}, "overall": []}
    ...
    # --- UTILIDADES ---
    def _normalize_score(value, min_val, max_val, reverse=False):
        if max_val == min_val:
            return 100
    ...
    def _calculate_stats(self, target_user_id: int) -> Dict[str, Any]:
        ...
        all_users = self.session.exec(select(Users)).all()
        gps_data = {gp.id: gp for gp in self.session.exec(select(GrandPrix)).all()}
        gps_dates = {
            gp.id: (
                gp.race_datetime.replace(tzinfo=timezone.utc)
                if gp.race_datetime.tzinfo is None
                else gp.race_datetime
            )
            for gp in gps_data.values()
        }

        part_counts = self.session.exec(
            select(Predictions.gp_id, func.count(Predictions.user_id)).group_by(
                Predictions.gp_id
            )
        ).all()
        gp_participation_map = {gp_id: count for gp_id, count in part_counts}

        race_results_db = self.session.exec(select(RaceResults)).all()
        official_results = {}
        ...
        metrics_raw = []
        for u in all_users:
            u_preds = self.session.exec(
                select(Predictions).filter(Predictions.user_id == u.id)
            ).all()
            if not u_preds:
                continue
            ...
            regularity_raw = (
                statistics.variance(points_list) if len(points_list) >= 3 else 999999
            )
            ...
            relevant_gps = sum(
                1
                for d in gps_dates.values()
                if (d if d.tzinfo else d.replace(tzinfo=timezone.utc))
                > (
                    user_created_at
                    if user_created_at.tzinfo
            ...
            for p in u_preds:
                ...
                if gp_date:
                    p_date = (
                        p.updated_at.replace(tzinfo=timezone.utc)
                        if p.updated_at.tzinfo is None
                        else p.updated_at
                    )
                    gp_dt = (
                        gp_date.replace(tzinfo=timezone.utc)
                        if gp_date.tzinfo is None
                        else gp_date
                    )
            ...
            weighted_sum = sum(
                (p.points * gp_participation_map.get(p.gp_id, 1)) for p in u_preds
            )
            ...
            for p in u_preds:
                ...
                for pp in p.positions:
                    possible += 1
                    if official["positions"].get(pp.position) == pp.driver_name:
                        hits += 1
                for pe in p.events:
                    possible += 1
                    if (
                        official["events"].get(pe.event_type, "").lower()
                        == pe.value.lower()
                    ):
                        hits += 1
        ...
        target_preds = self.session.exec(
            select(Predictions).filter(Predictions.user_id == target_user_id)
        ).all()
        # Ordenar
        target_preds_sorted = sorted(
            target_preds, key=lambda p: gps_dates.get(p.gp_id, datetime.min)
        )
        ...
        for p in target_preds:
            better = self.session.exec(
                select(func.count(Predictions.id)).filter(
                    Predictions.gp_id == p.gp_id, Predictions.points > p.points
                )
            ).scalar()
            if better == 0:
                trophies["gold"] += 1
        ...
        podium_ratio_percent = (
            int(
                (
                    (trophies["gold"] + trophies["silver"] + trophies["bronze"])
                    / races_played
                    * 100
                )
            )
        ...
        if races_played > 0:
            # Hero
            hero = self.session.exec(
                select(
                    PredictionPositions.driver_name,
                    func.count(PredictionPositions.driver_name).label("c"),
                )
                .join(Predictions)
                .filter(
                    Predictions.user_id == target_user_id,
                    PredictionPositions.position <= 3,
                )
                .group_by(PredictionPositions.driver_name)
                .order_by(desc("c"))
            ).first()
            if hero:
                insights["hero"] = {"code": hero[0], "count": hero[1]}

            # Villain
            villain = self.session.exec(
                select(
                    PredictionEvents.value,
                    func.count(PredictionEvents.value).label("c"),
                )
                .join(Predictions)
                .filter(
                    Predictions.user_id == target_user_id,
                    PredictionEvents.event_type == "DNF_DRIVER",
                )
                .group_by(PredictionEvents.value)
                .order_by(desc("c"))
            ).first()
            if villain:
                insights["villain"] = {"code": villain[0], "count": villain[1]}
            ...
            if bgp:
                ...
                worse = self.session.exec(
                    select(func.count(Predictions.id)).filter(
                        Predictions.gp_id == best_p.gp_id,
                        Predictions.points < best_p.points,
                    )
                ).scalar()
                perc = 100
                if total_in_race > 1:
                ...
                insights["best_race"] = {
                    "gp_name": bgp.name,
                    "year": bgp.season.year if bgp.season else 2026,
                    "points": best_p.points,
                    "percentile": f"Top {max(1, perc)}%",
                }
        ...
        if metrics_raw:
            ...
            if my_m:
                radar_data = [
                    {
                        "subject": "Regularidad",
                        "A": self._normalize_score(
                            my_m["reg"], min(regs), max(regs), reverse=True
                        ),
                        "fullMark": 100,
                    },
                    {
                        "subject": "Compromiso",
                        "A": self._normalize_score(my_m["com"], min(coms), max(coms)),
                        "fullMark": 100,
                    },
                    {
                        "subject": "Anticipación",
                        "A": self._normalize_score(my_m["ant"], min(ants), max(ants)),
                        "fullMark": 100,
                    },
                    {
                        "subject": "Calidad/Podios",
                        "A": self._normalize_score(my_m["pod"], min(pods), max(pods)),
                        "fullMark": 100,
                    },
                    {
                        "subject": "Vidente",
                        "A": self._normalize_score(my_m["vid"], min(vids), max(vids)),
                        "fullMark": 100,
                    },
                ]
    ...
    def get_all_users_light(self):
        ...
        query = select(
            Users.id, Users.username, Users.acronym, Users.avatar, Users.created_at
        )
    ...
    def get_my_stats(self, current_user: Users):
        res = self._calculate_stats(current_user.id)

        return res

    def get_user_stats(self, user_id: int):
        # Verificar si usuario existe
        u = self.session.get(Users, user_id)
        if not u:
