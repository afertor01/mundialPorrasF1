...
class StandingsRepository:
    ...
    def individual_season_standings(self, season_id: int) -> List[SeasonStandingResponse]:
        query = select(Users.id, Users.username, func.coalesce(func.sum(Predictions.points), 0).label("points")) \
        ...
        return [SeasonStandingResponse(user_id=row.id, username=row.username, points=row.points) for row in results]

    def gp_standings(self, gp_id: int) -> List[GPStandingResponse]:
        query = select(Users.id, Users.username, Predictions.points) \
            .join(Predictions, Predictions.user_id == Users.id) \
            .filter(Predictions.gp_id == gp_id) \
            .order_by(Predictions.points.desc())
        
        results = self.session.exec(query).all()
        return [GPStandingResponse(user_id=row.id, username=row.username, points=row.points) for row in results]
    
    def team_standings(self, season_id: int) -> List[TeamStandingResponse]:
        query = select(Teams.id, Teams.name, func.coalesce(func.sum(Predictions.points), 0).label("points")) \
        ...
        return [TeamStandingResponse(team_id=row.id, team_name=row.name, points=row.points) for row in results]
