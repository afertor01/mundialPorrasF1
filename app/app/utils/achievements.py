...
def recalculate_user_stats(session: Session, user_id: int, current_gp: GrandPrix) -> UserStats:
    ...
    query = select(func.sum(Predictions.points)).join(GrandPrix).where(Predictions.user_id == user_id, GrandPrix.season_id == current_gp.season_id)
    ...
    query = select(func.count(PredictionPositions.id))\
        .join(Predictions).join(RaceResults, Predictions.gp_id == RaceResults.gp_id)\
        .join(RacePositions, and_(
    ...
    query = select(Predictions).join(RaceResults, Predictions.gp_id == RaceResults.gp_id)\
    .filter(Predictions.user_id == user_id)\
    .options(
        joinedload(Predictions.events),
        joinedload(Predictions.positions),
        joinedload(Predictions.grand_prix).joinedload(GrandPrix.race_result).joinedload(RaceResult.events),
        joinedload(Predictions.grand_prix).joinedload(GrandPrix.race_result).joinedload(RaceResult.positions)
    )
...
def check_event_achievements(session: Session, user_id: int, gp: GrandPrix) -> Set[str]:
    ...
    query = select(Predictions).where(Predictions.user_id == user_id, Predictions.gp_id == gp.id)
    ...
    # Join Team
    query = select(TeamMembers).where(TeamMembers.user_id == user_id, TeamMembers.season_id == gp.season_id)
    has_team = session.exec(query).first()
    if has_team: unlocks.add("event_join_team")

    return unlocks

def check_career_season_achievements(session: Session, user_id: int, stats: UserStats) -> Set[str]:
...
def check_season_finale_achievements(session: Session, user_id: int, season_id: int) -> Set[str]:
    """
    Logros que SOLO se dan al cerrar la temporada (Campeón, Mochila).
    """
    unlocks = set()
    
    query = select(UserStats).filter(UserStats.last_gp_played_id.isnot(None)).order_by(desc(UserStats.current_season_points))
    ...
    if stat_entry:
        query = select(TeamMembers).where(TeamMembers.user_id == user_id, TeamMembers.season_id == season_id)
        tm = session.exec(query).first()
        if tm:
            query = select(TeamMembers.user_id).where(TeamMembers.team_id == tm.team_id, TeamMembers.season_id == season_id)
...
def verify_historical_validity(session: Session, user_id: int, slug: str) -> bool:
    ...
    if slug == "event_25pts": return session.exec(select(Predictions).where(Predictions.user_id==user_id, Predictions.points > 25)).first() is not None
    if slug == "event_50pts": return session.exec(select(Predictions).where(Predictions.user_id==user_id, Predictions.points > 50)).first() is not None
    if slug == "event_maldonado": return session.exec(select(Predictions).where(Predictions.user_id==user_id, Predictions.points == 0)).first() is not None
    if slug == "event_high_five": return _has_gp_with_exact_hits(5)
    if slug == "event_la_decima": return _has_gp_with_exact_hits(10)
    if slug == "event_join_team": return session.exec(select(TeamMembers).where(TeamMembers.user_id == user_id)).first() is not None
    if slug == "event_first": return session.exec(select(Predictions).where(Predictions.user_id == user_id)).count() >= 1
...
def evaluate_race_achievements(session: Session, gp_id: int):
    ...
    if not gp: return
    users = [u[0] for u in session.exec(select(Predictions.user_id).where(Predictions.gp_id == gp_id).distinct()).all()]
    # print(f"🔄 Sync Logros ({gp.name}) para {len(users)} usuarios...")
    for uid in users: sync_achievements(session, uid, gp)
...
def evaluate_season_finale_achievements(session: Session, season_id: int):
    ...
    for user in all_users:
        ...
        if slugs: 
            grant_achievements(session, user.id, list(slugs), season_id=season_id, gp_id=None)