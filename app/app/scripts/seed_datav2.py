...
def create_users_and_teams(session: Session, season):
    ...
    for i in range(NUM_USERS - 2):
        ...
        rand = random.random()
        if rand > 0.85:
            skill = random.uniform(0.75, 0.85)  # Expertos
        elif rand > 0.40:
            skill = random.uniform(0.40, 0.65)  # Normales
        else:
            skill = random.uniform(0.15, 0.35)  # Novatos
...
def simulate_race(session: Session, season, users, gp_index, all_driver_codes, user_skills):
    ...
    session.commit()  # Commit al final de la carrera
    sys.stdout.write(" OK\n")
