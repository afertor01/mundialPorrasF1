...
class GrandPrixRouter:
    ...
    def router(self) -> APIRouter:
        ...
        @api_router.post("/", description="Create a new Grand Prix", response_model=GrandPrix, dependencies=[Depends(require_admin)])
        def create_grand_prix(
            gp_data: GrandPrixCreateRequest,
            gp_service: GrandPrixServiceDep
        ) -> GrandPrix:
            return gp_service.create_grand_prix(gp_data)
        
        @api_router.get("/season/{season_id}", description="List all Grand Prix for a season", response_model=List[GrandPrix])
        def list_grand_prix(
            season_id: int,
            gp_service: GrandPrixServiceDep
        ) -> List[GrandPrix]:
