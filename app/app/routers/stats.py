...
class StatsRouter:
    ...
    def router(self) -> APIRouter:
        ...
        @api_router.get(
            "/evolution",
            description="Evolución de puntos o posiciones a lo largo de la temporada para pilotos o equipos específicos.",
            response_model=Dict[str, List[Dict[str, int | float]]],
        )
        def evolution(
            ...
            ids: List[int] = Query(
                ..., description="IDs de pilotos o equipos a comparar"
            ),
            names: List[str] = Query(
                ..., description="Nombres de pilotos o equipos a comparar"
            ),
            mode: str = Query(
                "points", description="Modo de cálculo (points/position)"
            ),
        ...
        @api_router.get(
            "/ranking",
            description="Ranking global de pilotos o equipos para una temporada específica.",
        ...
        @api_router.get(
            "/users",
            description="Lista ligera de usuarios para el buscador.",
        ...
        @api_router.get(
            "/me",
            description="Mis estadísticas detalladas comparadas con el global.",
        ...
        @api_router.get(
            "/user/{user_id}",
            description="Estadísticas detalladas de un usuario específico comparadas con el global.",
            response_model=Dict[str, Any],
            dependencies=[Depends(get_current_user)],
        )
        def get_user_stats(
            stats_service: StatsServiceDep, user_id: int
        ) -> Dict[str, Any]:
            return stats_service.get_user_stats(user_id)

        @api_router.get(
            "/achievements/{user_id}",
            description="Logros desbloqueados por un usuario específico.",
