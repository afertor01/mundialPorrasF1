...
class SeasonsRouter:
    ...
    def router(self) -> APIRouter:
        api_router = APIRouter(prefix="/seasons", tags=["Seasons"], dependencies=[Depends(get_current_user)])

        @api_router.get("/", description="Get all seasons", response_model=List[SeasonResponse])
        def get_seasons(
            seasons_service: SeasonsServiceDep
        ) -> List[SeasonResponse]:
            return seasons_service.get_seasons()

        @api_router.get("/{season_id}/teams", description="Get teams for a specific season", response_model=List[TeamResponse])
        ...
        @api_router.get("/{season_id}/constructors", description="Get constructors for a specific season", response_model=List[Constructors])
