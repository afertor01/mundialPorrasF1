...
class AvatarsRouter:
        ...
        def router(self) -> APIRouter:
            ...
            @api_router.get("/", description="Get all avatars", response_model=List[AvatarResponse])
            ...
            @api_router.put("/me/{avatar_filename}", description="Change my avatar", response_model=UserResponse)
            def select_avatar(
                avatar_filename: str,
            ...
            @api_router.post("/upload", description="Upload a new avatar", response_model=AvatarResponse, dependencies=[Depends(require_admin)])
            def upload_avatar(
                avatars_service: AvatarsServiceDep,
            ...
            @api_router.delete("/{avatar_id}", description="Delete an avatar", response_model=Dict[str, str], dependencies=[Depends(require_admin)])
            def delete_avatar(
                avatar_id: int,
                avatars_service: AvatarsServiceDep,
            ) -> Dict[str, str]:
