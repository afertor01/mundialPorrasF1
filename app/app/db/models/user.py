...
class Users(SQLModel, table=True):
    ...
    hashed_password: str = Field(
        description="Contraseña hasheada del usuario", nullable=False
    )
    role: str = Field(description="Rol del usuario", default="user")
    avatar: str = Field(
        description="Avatar del usuario", default="default.png", nullable=True
    )
    ...
    team_memberships: List["TeamMembers"] = Relationship(
        back_populates="user", cascade_delete=True
    )
    bingo_selections: List["BingoSelections"] = Relationship(
        back_populates="user", cascade_delete=True
    )
    achievements: List["UserAchievements"] = Relationship(
        back_populates="user", cascade_delete=True
    )
