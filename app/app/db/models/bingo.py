...
class BingoTiles(SQLModel, table=True):
    ...
    id: int = Field(
        description="ID de la casilla del bingo autogenerada", primary_key=True
    )
    season_id: int = Field(
        description="ID de la temporada a la que pertenece esta casilla",
        foreign_key="seasons.id",
    )
    description: str = Field(
        description="Descripción de la casilla del bingo", nullable=False
    )

    # Si es True, el evento ha ocurrido. Si es False, aún no (o no ocurrió al final)
    is_completed: bool = Field(
        description="Indica si el evento de la casilla ha ocurrido", default=False
    )
    ...
    selections: List["BingoSelections"] = Relationship(
        back_populates="tile", cascade_delete=True
    )
...
class BingoSelections(SQLModel, table=True):
    ...
    user_id: int = Field(
        description="ID del usuario que ha elegido esta casilla",
        foreign_key="users.id",
        primary_key=True,
    )
    bingo_tile_id: int = Field(
        description="ID de la casilla de bingo elegida",
        foreign_key="bingo_tiles.id",
        primary_key=True,
    )
