...
class Predictions(SQLModel, table=True):
    ...
    user_id: int = Field(
        description="ID del usuario que hizo la predicción",
        foreign_key="users.id",
        nullable=False,
    )
    gp_id: int = Field(
        description="ID del Grand Prix al que pertenece la predicción",
        foreign_key="grand_prix.id",
        nullable=False,
    )
    points_base: int = Field(description="Puntos base de la predicción", default=0)
    multiplier: float = Field(
        description="Multiplicador de puntos de la predicción", default=1.0
    )
    ...
    updated_at: datetime = Field(
        description="Fecha de actualización de la predicción",
        sa_column_kwargs={
            "server_default": func.now(),
            "onupdate": func.now(),
        },
    ...
    positions: List["PredictionPositions"] = Relationship(
        back_populates="prediction", cascade_delete=True
    )
    events: List["PredictionEvents"] = Relationship(
        back_populates="prediction", cascade_delete=True
    )
